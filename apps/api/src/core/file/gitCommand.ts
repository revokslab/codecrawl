...
export const execGitShallowClone = async (
  ...
  try {
    ...
    throw new Error(`Invalid repository URL. Please provide a valid URL. url: ${url}`)
  }
  ...
  if (remoteBranch) {
    ...
    try {
      ...
      const isRefNotFoundError =
        error instanceof Error && error.message.includes(`couldn't find remote ref ${remoteBranch}`)
